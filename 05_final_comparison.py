...
(Previously 06_final_comparison.py — renumbered for pipeline clarity)

Final comparison: Baseline vs Combined TDA+SD dataset.
Uses Data_ST for training and creates test set from Data_OG.

Run independently after the main pipeline has produced Data_ST.
"""
import sys
import torch
...
if __name__ == '__main__':
    ...
    for exp_name in experiments:
        if history[exp_name]['acc']:
            ...
            results.append({'Exp': exp_name, 'Trial': 'AVG',
                            'Acc': avg_acc, 'Prec': avg_prec, 'Rec': avg_rec,
                            'F1': avg_f1, 'MCC': avg_mcc, 'AUC': avg_auc})
            results.append({'Exp': exp_name, 'Trial': 'STD',
                            'Acc': std_acc, 'Prec': std_prec, 'Rec': std_rec,
                            'F1': std_f1, 'MCC': std_mcc, 'AUC': std_auc})
    ...
    pd.DataFrame(training_curves_data).to_csv(output_dir / 'training_curves.csv', index=False)
